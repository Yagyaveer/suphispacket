--!strict
-- Requires
local Signal = require(script.Parent.SignalSuhpi)
...
function Packet:Fire(...)
	...
	if self.ResponseReads then
		...
		threads[threads.Index] = {
			...
			Timeout = Task:Delay(self.ResponseTimeout, Timeout, coroutine.running(), self.ResponseTimeoutValue),
		}
...
function Packet:FireClient(player, ...)
	...
	Import(
		playerCursors[player]
			or { Buffer = buffer.create(128), BufferLength = 128, BufferOffset = 0, Instances = {}, InstancesOffset = 0 }
	)
	WriteU8(self.Id)
	if self.ResponseReads then
